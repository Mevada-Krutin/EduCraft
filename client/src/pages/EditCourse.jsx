...
const EditCourse = () => {
    ...
    useEffect(() => {
        const fetchCourse = async () => {
            try {
                ...
                setVideos(data.videos?.length ? data.videos : [{ title: '', url: '', duration: 0 }]);
    ...
    if (initialLoading) return <div className="text-center py-5">Loading Complete Data...</div>;

    return (
        <div className="animate-fade-in" style={{ maxWidth: '800px', margin: '2rem auto', padding: '0 1rem' }}>
            <h2 style={{ fontSize: '2rem', marginBottom: '2rem' }}>Edit Course</h2>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                    <button type="submit" disabled={saveLoading} className="btn btn-primary" style={{ padding: '1rem', fontSize: '1.1rem', flex: 1 }}>
                        {saveLoading ? 'Saving...' : 'Save Changes'}
                    </button>
                    <button type="button" onClick={() => navigate('/instructor/dashboard')} className="btn btn-outline" style={{ padding: '1rem', fontSize: '1.1rem', flex: 1 }}>
                        Cancel
                    </button>
                </div>
            </form>
